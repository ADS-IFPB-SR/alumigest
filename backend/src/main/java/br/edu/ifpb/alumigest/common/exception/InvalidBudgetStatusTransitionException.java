...
public class InvalidBudgetStatusTransitionException extends BusinessException {
    public InvalidBudgetStatusTransitionException(BudgetStatus current, BudgetStatus target) {
        super(String.format("Transição de status inválida: não é possível mudar de %s para %s.",
                current != null ? current.getDescricao() : "Indefinido",
                target != null ? target.getDescricao() : "Indefinido"));
    }
}