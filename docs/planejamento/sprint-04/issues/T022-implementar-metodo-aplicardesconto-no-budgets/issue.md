# [T022] Implementar método `aplicarDesconto()` no `BudgetService` com cálculo bidirecional (percentual ↔ valor fixo), validação de limites (desconto <= valorBruto) e persistência de condições de pagamento
...
- **Arquivo / Alvo Principal**: `BudgetService`
...
Implementar método `aplicarDesconto()` no `BudgetService` com cálculo bidirecional (percentual ↔ valor fixo), validação de limites (desconto <= valorBruto) e persistência de condições de pagamento.
...
- [ ] Implementar a alteração necessária em `BudgetService`
