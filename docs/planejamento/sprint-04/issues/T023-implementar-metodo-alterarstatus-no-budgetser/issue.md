# [T023] Implementar método `alterarStatus()` no `BudgetService` com máquina de estados (transições válidas conforme `data-model.md`)
...
Implementar método `alterarStatus()` no `BudgetService` com máquina de estados (transições válidas conforme `data-model.md`).
