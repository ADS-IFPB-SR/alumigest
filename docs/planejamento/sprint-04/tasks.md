...
| **US-09.25** | [US-09.25](issues/US-09.25-criar-budgetcontroller-com-endpoints-post-api/issue.md) Estender `BudgetController` com endpoints de aplicação de desconto (`PUT /discount`), adição de itens avulsos (`POST /items`) e alinhamento de rotas em `backend/src/main/java/br/edu/ifpb/alumigest/budgets/controller/BudgetController.java` | 🔲 Pendente |
