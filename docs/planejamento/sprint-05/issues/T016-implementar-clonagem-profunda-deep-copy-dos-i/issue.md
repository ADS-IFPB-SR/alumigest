# [T016] Implementar clonagem profunda (deep copy) dos itens do orçamento para `OrderItem` no `OrderService` garantindo lock de preços
...
- **Arquivo / Alvo Principal**: `OrderItem`
- **Labels Sugeridas**: `sprint-05`, `release-2`, `orders`, `backend`, `java`, `price-lock`, `mvp`, `user-story-1`

---

## 🎯 Objetivo & Descrição

Implementar clonagem profunda (deep copy) dos itens do orçamento para `OrderItem` no `OrderService` garantindo lock de preços.
...
- [ ] Implementar a alteração necessária em `OrderItem`
