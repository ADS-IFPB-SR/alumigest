# [T017] Implementar métodos `buscarPorId()` e `listar()` com paginação e filtros no `OrderService`
...
Implementar métodos `buscarPorId()` e `listar()` com paginação e filtros no `OrderService`.
