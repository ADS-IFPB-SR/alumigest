# [T014] Implementar métodos `buscarPorCodigo(String codigo)` e `listar(Pageable, status, orderId, busca)` no `ProductionOrderService`
...
- **Labels Sugeridas**: `sprint-06`, `release-2`, `production`, `backend`, `java`, `frontend`, `typescript`, `react`, `mvp`, `user-story-1`

---

## 🎯 Objetivo & Descrição

Implementar métodos `buscarPorCodigo(String codigo)` e `listar(Pageable, status, orderId, busca)` no `ProductionOrderService`.
