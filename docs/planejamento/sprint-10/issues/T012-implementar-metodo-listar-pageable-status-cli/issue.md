# [T012] Implementar método `listar(Pageable, status, clienteId, dataInicio, dataFim, busca)` no `AccountReceivableService` com atualização dinâmica de status `VENCIDO`
...
- **Arquivo / Alvo Principal**: `AccountReceivableService`
...
Implementar método `listar(Pageable, status, clienteId, dataInicio, dataFim, busca)` no `AccountReceivableService` com atualização dinâmica de status `VENCIDO`.
...
- [ ] Implementar a alteração necessária em `AccountReceivableService`
