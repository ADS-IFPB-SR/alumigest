# [T009] Criar script de restauração rápida `scripts/restore-backup.sh` e `scripts/restore-backup.ps1`
...
- **Arquivo / Alvo Principal**: `scripts/restore-backup.sh`
- **Labels Sugeridas**: `sprint-16`, `governance`, `security`, `audit`, `backup`, `devops`, `disaster-recovery`, `mvp`, `user-story-1`, `backup`

---

## 🎯 Objetivo & Descrição

Criar script de restauração rápida `scripts/restore-backup.sh` e `scripts/restore-backup.ps1`.
...
- [ ] Implementar a alteração necessária em `scripts/restore-backup.sh`
