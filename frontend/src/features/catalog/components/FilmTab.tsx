...
const columns = [
 { 
 header: 'Referência', 
 accessor: (row: FilmDTO) => <span className="font-data-mono text-data-mono text-on-surface-variant">{row.commercialReference || row.skuCode}</span>,
 exportValue: (row: FilmDTO) => row.commercialReference || row.skuCode
 },
 { 
 header: 'Descrição', 
 accessor: (row: FilmDTO) => <span className="font-title-sm text-title-sm text-on-surface font-semibold">{row.name}</span>,
 exportValue: (row: FilmDTO) => row.name
 },
 { 
 header: 'Preço Venda (m²)', 
 accessor: (row: FilmDTO) => <span className="font-data-mono text-data-mono text-on-surface">R$ {row.salePrice.toFixed(2).replace('.', ',')}</span>,
