...
const columns = [
 { 
 header: 'Referência', 
 accessor: (row: GlassDTO) => <span className="font-data-mono text-data-mono text-on-surface-variant">{row.commercialReference || row.skuCode}</span>,
 exportValue: (row: GlassDTO) => row.commercialReference || row.skuCode
 },
 { 
 header: 'Descrição', 
 accessor: (row: GlassDTO) => <span className="font-title-sm text-title-sm text-on-surface font-semibold">{row.name}</span>,
 exportValue: (row: GlassDTO) => row.name
 },
 { 
 header: 'Cor', 
 accessor: (row: GlassDTO) => <span className="text-secondary">{row.colorFinish}</span>,
 exportValue: (row: GlassDTO) => row.colorFinish
 },
 { 
 header: 'Espessura', 
 accessor: (row: GlassDTO) => <span className="font-data-mono text-data-mono text-secondary">{row.thicknessMm} mm</span>,
 exportValue: (row: GlassDTO) => `${row.thicknessMm} mm`
 },
 { 
 header: 'Preço Venda', 
 accessor: (row: GlassDTO) => <span className="font-data-mono text-data-mono text-on-surface">R$ {(row.salePrice || 0).toFixed(2).replace('.', ',')}</span>,
