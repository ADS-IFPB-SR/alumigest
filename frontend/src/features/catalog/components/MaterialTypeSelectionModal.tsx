...
export function MaterialTypeSelectionModal({
  isOpen,
  onClose,
  onSelect,
}: Props) {
  return (
    ...
    >
      <div data-cy="material-type-modal">
        <p className="font-body-sm text-body-sm text-on-surface-variant mb-xs">
          Escolha uma das categorias abaixo para abrir o formulário com os
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-sm sm:gap-md">
          {tipos.map((item) => (
            <button
              key={item.type}
              type="button"
              data-cy={`material-type-${item.type.toLowerCase()}`}
